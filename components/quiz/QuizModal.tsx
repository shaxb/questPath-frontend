    ...
    <div className="fixed inset-0 bg-gradient-to-b from-blue-600 to-purple-600 z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl">
        ...
        {!quizCompleted ? (
          <>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-6 text-white">
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className="text-white/80">Question {currentQuestionIndex + 1} of {quizData.questions.length}</p>
                </div>
                <button
                  onClick={handleClose}
            ...
            <div className="flex justify-center mb-6">
              <div className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold text-xl ${
                timeLeft <= 10 ? 'bg-red-500 animate-pulse' : 'bg-white/20 backdrop-blur-sm'
              } text-white`}>
                <Clock size={24} />
                {timeLeft}s
              </div>
            ...
            {/* Question Card */}
            <div key={currentQuestionIndex} className="bg-white rounded-3xl p-8 shadow-2xl mb-6 animate-slide-up">
                ...
                {quizData.questions[currentQuestionIndex].options.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-6 rounded-2xl text-left font-medium transition-all duration-300 ${
                        showCorrect
                          ...
                          : 'bg-gray-100 hover:bg-gray-200 border-4 border-transparent hover:border-blue-300 text-gray-900'
                      } ${showFeedback ? 'cursor-not-allowed' : 'cursor-pointer hover:scale-102'}`}
                    >
                      <div className="flex items-center gap-4">
                        <span className={`text-2xl font-black ${isSelected || showCorrect || showWrong ? 'text-white' : 'text-gray-700'}`}>{option.value}</span>
                        <span className="text-lg">{option.text}</span>
                        {showCorrect && <CheckCircle2 className="ml-auto" size={32} />}
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="flex justify-center">
              {!showFeedback ? (
                ...
                <button
                  ...
                  className="px-12 py-4 rounded-full font-bold text-xl bg-white text-blue-600 hover:scale-105 shadow-xl transition-all"
                >
          ...
          /* Results Screen */
          <div className="bg-white rounded-3xl p-12 shadow-2xl text-center animate-slide-up">
            ...
            {quizResults.passed && (
              <div className="bg-gradient-to-r from-yellow-100 to-orange-100 rounded-2xl p-6 mb-8">
                <div className="flex items-center justify-center gap-3 text-3xl font-black text-orange-600">
                  <Award size={40} />
            ...
            <p className="text-gray-600 mb-8">{quizResults.message}</p>

            <button
              onClick={handleClose}
              className="px-12 py-4 rounded-full font-bold text-xl bg-gradient-to-r from-blue-500 to-purple-500 text-white hover:scale-105 shadow-xl transition-all"
            >
              CONTINUE
            </button>
          </div>
